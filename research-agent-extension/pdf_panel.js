// pdf_panel.js — Auto-injected into PDF pages by background.js
// Creates a floating panel with minimize-to-bubble feature
...
(async () => {
  ...
  function createPanel(bubble) {
    ...
    output.innerText = "⏳ Extracting PDF text...";
  ...
  function showNavBar(ui, chunks, currentIdx, totalPages, onNavigate) {
    let navBar = ui.panel.querySelector(".ra-nav-bar");
    if (!navBar) {
      navBar = document.createElement("div");
      navBar.className = "ra-nav-bar";
      ...
      prevBtn.className = "ra-nav-prev";
      prevBtn.innerText = "\u25C0 Prev";
      Object.assign(prevBtn.style, {
        padding: "5px 14px", background: "#1976d2", color: "white",
        border: "none", borderRadius: "4px", cursor: "pointer",
        fontWeight: "500", fontSize: "12px"
      });

      const label = document.createElement("span");
      label.className = "ra-nav-label";
      Object.assign(label.style, { fontSize: "12px", color: "#555", minWidth: "140px", textAlign: "center" });

      const nextBtn = document.createElement("button");
      nextBtn.className = "ra-nav-next";
      ...
      const footer = ui.panel.lastElementChild;
      ui.panel.insertBefore(navBar, footer);
    }

    navBar.style.display = "flex";
    navBar._onNavigate = onNavigate;
    updateNavBar(ui, chunks, currentIdx, totalPages);
  }

  function updateNavBar(ui, chunks, idx, totalPages) {
    const navBar = ui.panel.querySelector(".ra-nav-bar");
    if (!navBar) return;

    const prevBtn = navBar.querySelector(".ra-nav-prev");
    const nextBtn = navBar.querySelector(".ra-nav-next");
    const label = navBar.querySelector(".ra-nav-label");

    label.innerText = `Pages ${chunks[idx].startPage}\u2013${chunks[idx].endPage} of ${totalPages}`;
    ...
    nextBtn.style.opacity = idx === chunks.length - 1 ? "0.4" : "1";
    nextBtn.style.cursor = idx === chunks.length - 1 ? "default" : "pointer";

    prevBtn.onclick = () => { if (idx > 0) navBar._onNavigate(idx - 1); };
    nextBtn.onclick = () => { if (idx < chunks.length - 1) navBar._onNavigate(idx + 1); };
  }

  function hideNavBar(ui) {
    const navBar = ui.panel.querySelector(".ra-nav-bar");
  ...
  function renderMarkdownPanel(md) {
    let text = md.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    const sections = [];
    // Match both **1. Title:** (bolded) and 1. Title: (unbolded) at line start or after whitespace
    const sectionRegex = /(?:^|\n)\s*(?:\*\*)?(\d+)\.\s+(.+?):?\s*(?:\*\*)?:?\s*(?=\n|$)/gm;
    let lastIdx = 0, match;
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (sec.type === "text" && i === 0) { html += `<p>${fmtInline(sec.content)}</p>`; continue; }
      if (sec.type === "heading") {
        const body = (i + 1 < sections.length && sections[i + 1].type === "text") ? sections[++i].content : "";
        const cls = sec.title.toLowerCase().includes("related") ? "related-works" : "section-card";
        html += `<div class="${cls}"><h2>${sec.num}. ${fmtInline(sec.title)}</h2>${body ? fmtBody(body) : ""}</div>`;
      }
    }
    return html;
  }
  ...
  function buildPageChunks(pages, maxChars) {
    const chunks = [];
    let chunkText = "", startPage = 1;
    const maxPagesPerChunk = 2;
    let pageCount = 0;
    for (let i = 0; i < pages.length; i++) {
      if ((chunkText.length + pages[i].length > maxChars || pageCount >= maxPagesPerChunk) && chunkText.length > 0) {
        chunks.push({ startPage, endPage: i, text: chunkText });
        chunkText = "";
        pageCount = 0;
        startPage = i + 1;
      }
    ...
    if (chunkText) {
      chunks.push({ startPage, endPage: pages.length, text: chunkText });
    }
    return chunks;
  }
  ...
  async function runAnalysis(ui) {
    ...
    setStatus(statusDot, statusText, "Reading PDF...", "loading");
    output.innerText = "⏳ Extracting text from PDF...";

    let extractResult;
    try {
      extractResult = await extractPDFText(window.location.href);
    } catch (err) {
      console.error("PDF extraction error:", err);
      setStatus(statusDot, statusText, "PDF extraction failed", "error");
      output.innerText = "❌ Could not read this PDF.\n\nError: " + err.message +
        "\n\nTips:\n• Make sure the extension has 'Allow access to file URLs' enabled in edge://extensions\n• Try opening the PDF via http:// instead of file://";
      return;
    }

    const { fullText, pages } = extractResult;

    if (!fullText || fullText.trim().length < 50) {
      setStatus(statusDot, statusText, "Scanned PDF — no text found", "error");
      output.innerText = "⚠️ This PDF is a scanned image.\n\nText extraction is not possible on image-only PDFs. Only text-based PDFs are supported.";
      return;
    }
    ...
    if (isResearch) {
      hideNavBar(ui);
      setStatus(statusDot, statusText, "Analyzing Research Paper...", "loading");
      output.innerText = "⏳ Sending to AI backend...\n\nDetected type: Research Paper";

      try {
        const data = await sendToBackend(truncated, "research");
        setStatus(statusDot, statusText, "✅ Research Paper analyzed", "success");
        output.innerHTML = renderMarkdownPanel(data.analysis);
      } catch (err) {
    ...
    // Normal document — page-wise chunked analysis
    const chunks = buildPageChunks(pages, MAX_CHARS);

    if (chunks.length <= 1) {
      hideNavBar(ui);
      setStatus(statusDot, statusText, "Analyzing Document...", "loading");
      output.innerText = "⏳ Sending to AI backend...\n\nDetected type: Document";

      try {
        const data = await sendToBackend(truncated, "normal");
        setStatus(statusDot, statusText, "✅ Document analyzed", "success");
        output.innerHTML = renderMarkdownPanel(data.analysis);
      } catch (err) {
        console.error("Backend connection error:", err);
        setStatus(statusDot, statusText, "Backend not running", "error");
    ...
    // Large document — paginated summarization
    const cache = {};
    let currentIdx = 0;

    async function analyzeChunk(idx) {
      currentIdx = idx;
      updateNavBar(ui, chunks, idx, pages.length);

      if (cache[idx]) {
        output.innerHTML = cache[idx];
        setStatus(statusDot, statusText, `\u2705 Pages ${chunks[idx].startPage}\u2013${chunks[idx].endPage} of ${pages.length}`, "success");
        return;
      }

      setStatus(statusDot, statusText, `Analyzing pages ${chunks[idx].startPage}\u2013${chunks[idx].endPage}...`, "loading");
      output.innerText = `⏳ Analyzing pages ${chunks[idx].startPage}\u2013${chunks[idx].endPage} of ${pages.length}...`;

      try {
        const data = await sendToBackend(chunks[idx].text, "normal");
        const html = renderMarkdownPanel(data.analysis);
        cache[idx] = html;
    ...
    showNavBar(ui, chunks, 0, pages.length, analyzeChunk);
    analyzeChunk(0);

    ui.retryBtn.onclick = () => {
      delete cache[currentIdx];
      analyzeChunk(currentIdx);
    };
  }

  // ─── INIT ────────────────────────────────────────────────

  // Create bubble first (hidden), then panel
  const bubble = createBubble(() => {
    ui.panel.style.display = "flex";
    ui.panel.style.animation = "ra-panel-in 0.3s ease-out";
  });
  bubble.style.display = "none"; // hidden initially since panel is open
